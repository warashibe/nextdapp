...
const setETH = async ({ val: { _network }, set, state$ }) => {
  ...
  let obj = {
    ...
    eth_selected: eth_selected,
    web3_block: block
  }
  if (R.xNil(eth_selected) && R.isNil(state$.value.address_in_use)) {
    obj.address_in_use = "eth"
  } else if (R.isNil(eth_selected)) {
    obj.address_in_use = R.isNil(state$.value.auth_selected) ? null : "auth"
  }
  obj.eth_updated = Date.now()
  set(obj, null)
}
...
const hookAuthereum = async ({ val: { _network }, set, state$ }) => {
  ...
  const block = await web3_authereum.eth.getBlockNumber()
  set(block, "web3_block")
