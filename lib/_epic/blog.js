...
const _3box_getSpace = async ({ set, state$, val: { _address, Box } }) => {
  ...
  if (R.isNil(space)) {
    const box = await Box.openBox(my_address, provider)
    space = await box.openSpace(`next-dapp-blog_${_address}`)
    set(space, [
      "blog_3box",
      "spaces",
...
export const connect_to_3box = async ({
  ...
  try {
    const subs = await space.subscribedThreads()
    if (subs.length > 50) {
      for (let v of subs) {
        await space.unsubscribeThread(v.address)
      }
    }
