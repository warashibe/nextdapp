...
export const blogChangeListTitleFB = epic(
  "blogChangeListTitleFB",
  async ({ state$, val: { user, title, id }, set }) => {
    await fb().fsdb.update({ title: title }, "article_lists", id)
    let blog_dirs = state$.value.blog_posted_dirs || []
    for (let v of blog_dirs) {
      if (v.id === id) {
        v.title = title
      }
    ...
    set(
      {
        blog_posted_dirs: blog_dirs,
        blog_note: { text: "Saved!" },
        blog_updated: Date.now(),
...
const _3box_getBox = ({ state$, set, pin = "none" }) => {
  let Box = state$.value.blog_3box.Box[pin]
  if (R.isNil(Box)) {
    Box = require("3box")
    set(Box, ["blog_3box", ["Box", pin]])
  }
  return Box
}
const _3box_getSpace = async ({
  ...
  if (R.isNil(space)) {
    ...
    const Box = _3box_getBox({
      state$,
      set,
      pin: conf.blog.pinningNode || "none"
    })
...
export const connect_to_3box = async ({
  ...
  try {
    space = await new Promise((ret, rej) => {
      setTimeout(() => {
      ...
      _3box_getSpace({ set, state$, val: { _address } }).then(_space => {
        ret(_space)
      })
    })
  } catch (e) {
  ...
  try {
    if (R.xNil(error_space) && error_space === true) {
      space = await _3box_getSpace({
        set,
        state$,
        val: { _address, nopin: true }
      })
    }
...
export const createArticleListFB = epic(
  "createArticleListFB",
  async ({ state$, val: { user }, set }) => {
    set(true, "blog_loading")
    const id = shortid.generate()
    const new_list = { id: id, title: id, uid: user.uid }
    const lists = await fb().fsdb.set(new_list, "article_lists", id)
    let dirs = state$.value.blog_posted_dirs || []
    let dirmap = {}
    for (let v of dirs) {
      dirmap[v.id] = true
    }
    if (R.isNil(dirmap[id])) {
      dirs.push(new_list)
    }
    set(dirs, "blog_posted_dirs")
    set({ text: "Created!" }, "blog_note")
    set(Date.now(), "blog_updated")
    set(false, "blog_loading")
  }
...
export const getMyArticles = epic(
  "getMyArticles",
  async ({
    set,
    val: { user, published = true, nochange = false, topics = {} }
  }) => {
    const articles = await fb().fsdb.get(
      "articles",
      ["user.uid", "==", user.uid],
      ["published", "==", published]
    )
    const lists = await fb().fsdb.get("article_lists", ["uid", "==", user.uid])
    let dirs = []
    let dirmap = {}
    for (let v of articles) {
      if (
        R.xNil(v.topic) &&
        R.isNil(dirmap[v.topic]) &&
        R.xNil(topics[v.topic])
      ) {
        dirmap[v.topic] = true
        topics[v.topic].predefined = true
        dirs.push(topics[v.topic])
      }
    }
    for (let v of lists) {
      if (R.isNil(dirmap[v.id])) {
        dirs.push(v)
        dirmap[v.id] = true
      }
    }
    ...
    for (let v of articles) {
      if (R.isNil(v.topic)) {
        v.unsorted = true
      } else if (R.isNil(dirmap[v.topic]) && R.isNil(unknowns[v.topic])) {
        unknowns[v.topic] = v.topic
        dirs.push({
          title: v.topic,
          predefined: true,
          unknown: true,
    ...
    if (nochange === false) {
      set(published ? "public" : "private", "blog_posted_access")
      set(dirs, "blog_posted_dirs")
    }
    set(articles, "blog_posted_articles")
  }
)
export const getArticle = epic("getArticle", async ({ set, val: { id } }) => {
  const article = await fb().fsdb.get("articles", id)
  if (R.xNil(article)) {
    set(article.topic, "blog_topic")
    set(article.cover, "blog_cover")
  } else {
    set("etc", "blog_topic")
    set(null, "blog_cover")
  }
...
export const postToWarashibe = epic(
  "postToWarashibe",
  async ({
    set,
    val: { topic, cover, title, body, user, id, published = true }
  }) => {
    set(true, `ongoing_postToWarashibe_${id}`)
    const exist = await fb().fsdb.get("articles", id)
    const $ = cheerio.load(`<div id="main">${converter.makeHtml(body)}</div>`)
    const overview = $("#main")
      .text()
      .slice(0, 50)
    const date = Date.now()
    ...
    if (R.isNil(exist)) {
      ...
      article.created_at = exist.created_at
    }
    const ref = fb()
      .storage.ref()
      .child(`articles/${id}.json`)
    const uploadTask = await ref.putString(
      JSON.stringify(R.mergeLeft({ body: body }, article))
    )
    await fb().fsdb.upsert(article, "articles", id)
    set(false, `ongoing_postToWarashibe_${id}`)
    set(article, "blog_article")
    set({ text: "投稿しました！" }, `blog_note`)
    //window.location.href = `/articles/${id}`
  }
)
export const deleteFromWarashibe = epic(
  "deleteFromWarashibe",
  async ({ set, val: { id } }) => {
    set(true, `ongoing_deleteFromWarashibe_${id}`)
    const ref = fb()
      .storage.ref()
      .child(`articles/${id}.json`)
    const uploadTask = await ref.delete()
    await fb().fsdb.delete("articles", id)
    set(false, `ongoing_deleteFromWarashibe_${id}`)
    set(null, `blog_article`)
  }
)

export const loadPostedArticle = epic(
  "loadPostedArticle",
  async ({ set, val: { id } }) => {
    set(true, "blog_loading")
    const url = `https://firebasestorage.googleapis.com/v0/b/${
      conf.firebase.id
    }.appspot.com/o/articles%2F${id}.json?alt=media`
    ...
    set(
      {
        blog_posted: true,
        blog_new_title: article.title,
        blog_new_title_lock: article.title,
        blog_new_body: article.body,
        blog_new_body_lock: article.body,
        blog_editor_value: toValue(article.body),
        blog_history: [],
        blog_post_history: [],
        blog_history_cursor: -1,
        blog_selected_article: id,
        blog_loading: false,
        blog_updated: Date.now()
      },
      null
...
export const createBlankArticle = epic(
  "createBlankArticle",
  async ({ set, val: {}, state$ }) => {
    const id = shortid.generate()
    set(-1, "blog_history_cursor")
...
export const init = {
  ...
  blog_updates: {},
  blog_loading: false,
  blog_3box: { Box: {}, spaces: {}, threads: {} },

  blog_posted: true,
  blog_cover: null,
  blog_posted_selected_dir: null,
  blog_articles: [],
  blog_posted_articles: [],
  blog_posted_dirs: [],
  blog_article: [],
  blog_topic: "etc",
  blog_posted_access: "public"
}
