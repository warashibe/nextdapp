...
export const uniswap_tokens = epic(
  ...
  async ({
    type,
    val: { from, to, amount = 0, to_amount = 0 },
    dispatch,
    extra,
    state$,
    set
    ...
    if (lock === "from") {
      ...
      const [err, exchange_address] = await R.err(
        uniswap_factory.methods.getExchange(to || from).call,
        uniswap_factory
      )()
      const _err = (error, message = "something weng wrong.") => {
        if (R.isNil(error)) return false
      ...
      if (R.isNil(from)) {
        const [err2, tx] = await R.err(
          uniswap.methods.ethToTokenSwapOutput(
            web3.utils.toWei(to_amount.toString()),
            dead
          ).send,
          uniswap
        )({
          value: web3.utils.toWei((amount * 1.005).toString()),
          from: address
        })
        if (_err(err2)) return
      } else if (R.isNil(to)) {
        const [err2, tx] = await R.err(
          uniswap.methods.tokenToEthSwapOutput(
            web3.utils.toWei(N(to_amount).toFixed(18)),
            web3.utils.toWei(N(amount * 1.005).toFixed(18)),
            dead
          ).send,
          uniswap
        )({
        ...
        const [err, exchange_address2] = await R.err(
          uniswap_factory.methods.getExchange(from).call,
          uniswap_factory
        )()
...
const check_uniswap_rate = async ({
  ...
  if (lock === "from") {
    ...
    set(web3.utils.fromWei(price || "0"), `uniswap_${tar}_amount`)
  } else {
    const exchange_address = await uniswap_factory.methods
      .getExchange(to || from)
      .call()
